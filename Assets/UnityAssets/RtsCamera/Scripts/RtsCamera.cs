...
public class RtsCamera : MonoBehaviour
{
    ...
    protected void LateUpdate()
    {
        ...
        if (Smoothing)
        {
                ...
                _currRotation = Mathf.LerpAngle(_currRotation, Rotation, Time.unscaledDeltaTime * RotationDampening);
                _currDistance = Mathf.Lerp(_currDistance, Distance, Time.unscaledDeltaTime * ZoomDampening);
                _currTilt = Mathf.LerpAngle(_currTilt, Tilt, Time.unscaledDeltaTime * TiltDampening);
                _target.transform.position = Vector3.Lerp(_target.transform.position, LookAt, Time.unscaledDeltaTime * MoveDampening);
