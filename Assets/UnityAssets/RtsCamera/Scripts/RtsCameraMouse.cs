...
public class RtsCameraMouse : MonoBehaviour
{
    ...
    protected void Update()
    {
        ...
        if (AllowZoom)
        {
            ...
            _rtsCamera.Distance -= scroll * ZoomSpeed / 10;
        }
        ...
        if (AllowScreenEdgeMove && (!_rtsCamera.IsFollowing || ScreenEdgeMoveBreaksFollow))
        {
            ...
            {
                hasMovement = true;
                _rtsCamera.AddToPosition(0, 0, MoveSpeed / 10);
            }
            else if (Input.mousePosition.y < ScreenEdgeBorderWidth)
            {
                hasMovement = true;
                _rtsCamera.AddToPosition(0, 0, -1 * MoveSpeed / 10);
            }

            if (Input.mousePosition.x > (Screen.width - ScreenEdgeBorderWidth))
            {
                hasMovement = true;
                _rtsCamera.AddToPosition(MoveSpeed / 10, 0, 0);
            }
            else if (Input.mousePosition.x < ScreenEdgeBorderWidth)
            {
                hasMovement = true;
                _rtsCamera.AddToPosition(-1 * MoveSpeed / 10, 0, 0);
            }
